...
pub fn has_one_pair_same(password: &Vec<char>) -> bool {
    let mut total_same_group = 1;
    for i in 0..password.len() - 1 {
        if password[i] == password[i + 1] {
            total_same_group += 1;
        } else {
            if total_same_group == 2 {
                return true;
            }
            total_same_group = 1;
        }
...
pub fn good_password(password: &Vec<char>) -> bool {
    is_equal_length(&password, 6) && has_one_pair_same(&password) && !has_decrease(&password)
}
